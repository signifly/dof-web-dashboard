...
async function getDatabaseSchema() {
  ...
  try {
    ...
    const { data: sessionColumns, error: sessionError } = await supabase
      .rpc('get_table_schema', { table_name: 'performance_sessions' })
    ...
    if (sessionError) {
      ...
      if (sampleError) {
        ...
        const testColumns = [
          'id', 'anonymous_user_id', 'session_start', 'session_end',
          'app_version', 'device_type', 'os_version', 'created_at',
    ...
    const metricsTestColumns = [
      'id', 'session_id', 'metric_type', 'metric_value', 'metric_unit',
      'context', 'timestamp', 'created_at',
      'memory_usage', // This is the missing one
      'fps', 'cpu_usage', 'load_time' // These might also be missing
    ]
    ...
    console.log('  ❌ fps - MISSING from actual database (used in code but not in schema)')
    console.log('  ❌ cpu_usage - MISSING from actual database (used in code but not in schema)')
    console.log('  ❌ load_time - MISSING from actual database (used in code but not in schema)')

    console.log('\n💡 Root causes identified:')
    console.log('1. Database schema does not include columns expected by application code')
    console.log('2. Application expects denormalized columns (memory_usage, fps, etc.)')
    console.log('3. Actual database uses normalized approach (metric_type + metric_value)')
