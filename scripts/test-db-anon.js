...
async function testDatabaseWithAnon() {
  ...
  try {
    ...
    if (sessionsError) {
      console.log(
        `❌ Error accessing performance_sessions: ${sessionsError.message}`
      )

      // Check if it's a missing table or schema issue
      if (sessionsError.message.includes("does not exist")) {
        console.log(
          "   → This suggests the table does not exist in the database"
        )
      } else if (sessionsError.message.includes("permission")) {
        console.log(
          "   → This suggests a permissions issue with anonymous access"
        )
      }
    } else {
      console.log(
        `✅ performance_sessions accessible, found ${sessions?.length || 0} records`
      )
    ...
    if (metricsError) {
      console.log(
        `❌ Error accessing performance_metrics: ${metricsError.message}`
      )
      ...
      console.log(
        `✅ performance_metrics accessible, found ${metrics?.length || 0} records`
      )
      if (metrics && metrics.length > 0) {
        console.log(
          "   Sample record columns:",
          Object.keys(metrics[0]).join(", ")
        )
      }
    }
